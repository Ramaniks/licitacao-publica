...
class Controller_Edital extends Controller_Template_Website {
    ...
    public function action_index()
    {
		...
		$this->template->content = View::factory('edital/listar', $dataEditais); // application/views/admin/form.php
		//this->list();
	}
	...
	public function action_editaislist() {
		$users = ORM::factory('editais')->find_all()->as_array();
		# Add where clause, etc, here if filtering
		$pagination = new Pagination($users, $this->request->param('page'), 'admin_user');
		$this->template->content = View::Factory('edital/newpage', $pagination);;
	}
	...
	public function action_lista(){
        ...
        public function action_manutencao($id=NULL){
                               
				$id=$this->request->param('id');      
				$this->template->title = 'Listar Editais';
                //Seleciona um objeto Post através do ORM,
                //se nao existir id trás um objeto em branco ->find_all()         
                $data = ORM::Factory('editais', $id)->as_array();
