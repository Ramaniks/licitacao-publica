...
require ($_caminho_banco."conexao_postgre.php");
/*$db_url = "dbname= portal2009 port=5432 host = 192.168.1.90 user = portal2009 password = Portal2009";
...
if ($tipo_download == "1") {
	$sql = "select cd_edital, nr_ano, lk_edital, tp_edital, nm_edital, st_edital_pago from edital.editais where nr_registro= $identificacao";//NR_TAMANHO_EDITAL,
	$stmt_edital= pg_query($conexao_postgre, $sql);
	//ociexecute($stmt_edital);
	$row_edital= pg_fetch_assoc($stmt_edital);
	...
	if ($st_edital_pago == "S"){
		$sql_empresa = "SELECT 'S' as achou FROM edital.controle_empresas WHERE nr_edital= $cd_edital AND nr_ano = $nr_ano";
		if (!is_null($cod_empresa))
			{$sql_empresa = $sql_empresa. "  AND cd_empresa = $cod_empresa";}
		else
			{$sql_empresa = $sql_empresa. "  AND cd_empresa = null";}
		$stmt_empresa= pg_query($conexao_postgre, $sql_empresa);
		////ociexecute($stmt_empresa);
		$row_empresa= pg_fetch_assoc($stmt_empresa);
	...
	if ($download_liberado == "S") {
		...
		if ($resultado == false) {
			...
			echo "</script>";
			echo " * ".$tp_edital." - ".$VAR_PATH_UPLOAD.$lk_edital." * ".$edital_tamanho." - ".$nm_edital." - ".$nome_arquivo." lk_edital=".$lk_edital.".";
		}
		exit;
	}
} else {
	...
	$stmt_edital= pg_query($conexao_postgre, $sql);
	////ociexecute($stmt_edital);
	$row_edital= pg_fetch_array($stmt_edital);
