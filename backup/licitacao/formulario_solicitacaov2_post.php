...
$sql = "select cd_edital, nr_ano, lk_edital, nm_edital, cd_entidade, cd_modalidade from edital.editais where nr_registro= $nr_registro";
...
function fnc_grava_solicitacao () {
	...
	if (retorno) {
		...
		$("#div_retorno").load('formulario_solicitacao-gv2_post.php', {
...
function valida()
{
  ...
  if (f.nm_e_mail.value == "" || f.nm_e_mail.value.indexOf('@',0) == -1) {
	window.alert('Sr. Usuário, favor preencher corretamente o campo e-mail, com um e-mail válido!')
	f.nm_e_mail.focus();
	return false;
  }  
    ...
    <td>
	  <form method="post" name="edital" id="edital">
			...
			<input readonly name="nm_edital" type="hidden" id="nm_edital" value="<?php echo $nm_edital; ?>">
				...
				<div id='div_retorno' align='center'>div_retorno</div>
			</div>
	  </form>	
	</td>
  </tr>
